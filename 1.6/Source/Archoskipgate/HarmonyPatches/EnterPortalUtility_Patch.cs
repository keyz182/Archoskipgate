...
public static class EnterPortalUtility_Patch
{
    ...
    {
        List<Thing> doors = clickCell.GetThingList(pawn.Map).Where(t => t is Building_SkipGate or Building_SkipGateExit).ToList();
        if (doors.Count <= 0)
        {
            return true;
        }

        if (doors.First() is not Building_SkipGate portal)
        {
            return true;
        }
    ...
    {
        List<Thing> doors = clickCell.GetThingList(pawns[0].Map).Where(t => t is Building_SkipGate or Building_SkipGateExit).ToList();
        if (doors.Count <= 0)
        {
            return true;
        }
